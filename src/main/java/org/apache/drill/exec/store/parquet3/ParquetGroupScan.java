...
import org.apache.drill.exec.store.parquet3.Metadata.ColumnMetadata;
import org.apache.drill.exec.store.parquet3.Metadata.ParquetFileMetadata;
import org.apache.drill.exec.store.parquet3.Metadata.ParquetTableMetadataBase;
import org.apache.drill.exec.store.parquet3.Metadata.RowGroupMetadata;
...
public class ParquetGroupScan extends AbstractFileGroupScan {
  static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(
      org.apache.drill.exec.store.parquet3.ParquetGroupScan.class);
  static final MetricRegistry metrics = DrillMetrics.getInstance();
  static final String READ_FOOTER_TIMER = MetricRegistry.name(
      org.apache.drill.exec.store.parquet3.ParquetGroupScan.class, "readFooter");


  private final List<ReadEntryWithPath> entries;
  private final Stopwatch watch = Stopwatch.createUnstarted();
  private final ParquetFormatPlugin formatPlugin;
  private final org.apache.drill.exec.store.parquet3.ParquetFormatConfig formatConfig;
  ...
  @JsonCreator public ParquetGroupScan( //
    ...
    if (formatConfig == null) {
      formatConfig = new org.apache.drill.exec.store.parquet3.ParquetFormatConfig();
    }
   ...
   */
  private ParquetGroupScan(org.apache.drill.exec.store.parquet3.ParquetGroupScan that) {
    super(that);
    ...
    if (fileStatuses.size() == 1 && selection.getSelectionRoot().equals(first.toString())) {
      ...
      for (FileStatus status : fileStatuses) {
        if (status.isDirectory()) {
          ...
          final ParquetTableMetadataBase metadata = Metadata.readBlockMeta(fs, metaPath.toString());
  ...
  public PhysicalOperator getNewWithChildren(List<PhysicalOperator> children) {
    ...
    return new org.apache.drill.exec.store.parquet3.ParquetGroupScan(this);
  }
  ...
  public GroupScan clone(List<SchemaPath> columns) {
    org.apache.drill.exec.store.parquet3.ParquetGroupScan
        newScan = new org.apache.drill.exec.store.parquet3.ParquetGroupScan(this);
